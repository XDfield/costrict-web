...
| 关联文档 | `CATALOG_INGEST.md`、`SCAN_SKILL.md`、`DATABASE_DESIGN.md`、`HTTP_TUNNEL_DESIGN.md`、`IDENTITY_FEDERATION_DECISION.md`、`CAPABILITY_PORTAL_DECISION.md` |

> v2 变更：仓库布局从单 mono-repo 改为**按来源类型混合策略**；发现层走 server REST API 查 DB，不维护派生 Git 索引。
> v2.1 变更：去掉 `capability-index` 索引仓库与 index sync worker，发现层完全由 server DB + REST API 承担。
> v3 变更：用户中心主权从 Casdoor 移到 **costrict-web**（自签 JWT）；Casdoor 退化为多登录源 UI 提供者；Gitea fork 加 JWT 中间件（§9）；Branch Protection 简化为仅防 force push + delete（§9.8.2）；Bot 身份模型收敛到单一 `costrict-system` admin PAT（§9.7）。

---

## TL;DR

把 `capability_items`（Skill / Subagent / Command / MCP / Plugin 五类能力项）的**内容与版本真相源**从 PostgreSQL 迁移到基于 Gitea 的 Git 仓库；PostgreSQL 退化为运行时索引（缓存 + 业务字段）。每个能力项通过 `source_repo_url + source_repo_path + ref` 获得稳定的唯一可解析地址。**用户中心主权归 costrict-web**（自签 JWT，§9）；Casdoor 退化为多登录源 UI 提供者；Gitea fork 加 JWT 中间件（验证 costrict-web 签发的 JWT + auto-provisioning）；用户侧行为（csc push / AI agent 代用户 / API 调用）走用户自己 PAT 或 JWT，系统服务侧仅 `costrict-system` 单一 admin PAT 用于 2 个明确场景（capability 索引同步 + 用户生命周期级联，§9.7）。
...
6. **统一审核工作流（双轨制）**：默认通道为 **git push 直推 main**（§9.8.2 简化方案，与 V2 编辑 UX 一致）；**可选 PR 通道**用于公有能力贡献 / 跨业务线变更 / 重要能力项评审（AI 起草 → 人类审核 merge）；两条通道都触发 sync + health/security check；误推送靠 §8.4 健康度治理 + git revert 兜底
7. **消除 catalog bundle 中转**：上游 repo 通过 Gitea mirror 或 git remote 直接接入
8. **用户中心主权归 costrict-web**（自签 JWT + 业务字段），**Casdoor 退化为多登录源 UI 提供者**（GitHub OAuth / 短信 / LDAP 等社交登录入口），Gitea fork 加 JWT 中间件验证 costrict-web 签发的 JWT 并 auto-provision 用户（§9）；用户/AI 在 costrict-web 与 Gitea 之间通过同域 cookie + JWT 实现 SSO
...
- **不动 Casdoor 多源登录 UI 能力**（继续承担 GitHub OAuth / 短信 / LDAP 等社交登录入口），但**用户中心主权移到 costrict-web**——Casdoor 不再是 IdP，仅是登录方式选择器；用户身份 / 属性 / 角色由 costrict-web 主权管理并通过 webhook 广播（§9）
...
│   u-<username>/<slug>               用户个人 namespace 草稿区（默认 private）│
│                                                                             │
│   fork JWT 中间件（验证 JWT + auto-provisioning，§9.6）                      │
│   webhook ──push──► costrict-web server                                     │
└──────────────────────────────┬──────────────────────────────────────────────┘
                               │ Gitea REST API: compare / raw / trees
                               │ (`costrict-system` admin PAT，§9.7)
                               ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                       costrict-web server                                    │
│                                                                             │
...
> **关键约束**：sync handler 永远只读**能力项顶层 metadata 文件**（standalone 是 `skill.md` / `subagent.md` 等，pack 是 `plugins/<id>/.plugin.json`），不深入子目录解析内部 skill/command/mcp 文件。详见 §8.3。
>
...
└── u-<username>/                             # 用户个人 namespace（每个认证用户 1 个 owner）
    ├── u-alice/
    │   ├── skill-experiment                  # 用户自建 standalone（默认 private）
    │   └── draft-subagent
    └── u-bob/
        └── my-mcp-test
```
...
> - **用户 namespace**：`u-<username>/`，per-user auto-provision（Gitea 原生 user namespace）
>
> **`costrict-config/platform-config` 的特殊地位**：
> - GitOps 控制面板，所有 Gitea 全局配置的真相源
> - branch protection 稍严（仅 admin push；应急 force push 走 admin override，§9.9.4）
> - `GiteaConfigSyncWorker` 监听其 push webhook，自动 diff `.gitea/*.yaml` 变更并应用
>
> **用户 namespace 规则**：
> - 命名：`u-` 前缀 + 用户 username（避免与官方 org 命名冲突）
> - 创建：用户首次 push 触发 fork JWT 中间件 auto-provision Gitea user 后，Gitea 原生支持 user namespace
> - 默认 visibility：private（仅 owner 可见，可手动改 public）
> - 发现层默认过滤：marketplace 列表默认不展示 `u-*/*` 下 repo（仅 owner 自己的 `/my-drafts` 页面可见）
> - public 流程：用户提 PR 到 `costrict/<slug>` → admin merge 后转官方 org（§8.6 贡献流程）
...
**类型锁定后的变更**：必须先在 costrict-web 后台"申请类型变更"——审核通过后 unlock → 改文件 → re-sync。直接改文件结构会触发健康度告警（见 §8.4）。

**Mirror repo 特殊处理**：mirror pull 后 server 检测 upstream 文件结构变化（如 `SKILL.md` 消失）→ 标记 `polluted`（见 §8.4），由 owner 决定是否继续 mirror。
...
AI agent 任务："加一个 skill-vetter"
   ...
   ├─ git push origin main     # 用 scoped PAT（§9.8.3）
   │
   ├─ 主分支 push 触发 webhook
...
| 主体 | 凭据 | 范围 | 用途 |
|---|---|---|---|
| 人类用户 | Casdoor 多源登录 → costrict-web 自签 JWT → Gitea fork JWT 中间件（§9.2） | 按用户角色 | 浏览 Gitea / PR / 审核 / web UI 操作 |
| 用户（git 客户端 / csc / AI agent 代用户） | 用户自己 fine-grained PAT（D 方案 C 通道，§9.8.3） | `read:repository` + `write:repository` 限定 owner | git push / clone / API 操作（commit author = 用户本人） |
| 服务端 sync worker | `costrict-system` 单一 admin PAT（§9.7） | `read:repository` + 跨 owner 含 private | 调 compare / raw / trees API 拉 capability 内容 + 用户生命周期级联 |
...
- worker 流程：调 Gitea API 拉 PR head / commit 文件树 → 启发式识别（§4.5 规则）+ schema 校验 → 写入 `capability_items.health_issues` + `identification_status` → 调 Gitea API 在 PR 评论 health summary
...
- 迁移完成：删 `item_revision`（见 Stage 5）
...
**与 `identification_status` 的关系**：

- `identification_status`（§8.4）：文件结构健康度，capability-check worker 产出
...
**scan_service 不拆分**：暂不抽出 `LLMScanService` 与未来静态分析/依赖扫描解耦——保留现状，未来如有第二种扫描引擎再重构。
...
**不引入** `internal` / `hidden` 等中间态——业务线层级仅用于 costrict-web 发现层过滤（§9.10 E4 简化方案：dept-sync → Redis cache → API 过滤），Gitea 不掺业务 team 概念；草稿/未发布通过用户 namespace `u-<username>/` 下 private repo 表达。

**来源类型 × 默认 visibility**：

| kind / org | 默认 visibility | 可调整 |
|---|---|---|
| `costrict/` 官方 standalone | public | 否（admin 强制） |
...
| **`u-<username>/` 用户自建 standalone** | private（默认仅 owner） | 是（用户可在自己 namespace 内改 public；但 marketplace 默认过滤 `u-*/*`，进发现层需 PR 迁移到 `costrict/` org，见下方"公有能力贡献流程"） |

**Server sync worker 访问权限**：

- service account PAT scope = `read:repository`（全局，含 private repo）
- sync worker 扫描所有 repo（含 private）写入 `capability_items`
- `capability_items` 表本身**不存 visibility 字段**——发现层 API 实时调 Gitea 反查

**发现层 API 过滤逻辑**（`GET /api/capabilities`）：

```
def list_capabilities(user, ...):
    # 双层过滤
    public_items = SELECT * FROM capability_items ci
                    JOIN capability_registries cr ON ci.source_repo_url = cr.git_remote_url
                    WHERE cr.gitea_visibility = 'public'
    
    private_repos = gitea_api.list_user_repos(user)  # 含 team membership
    private_items = SELECT * FROM capability_items
                     WHERE source_repo_url IN (private_repos)
    
    return public_items + private_items
```

- `public` 部分：直接返回（server 缓存 `capability_registries.gitea_visibility` 字段，sync worker 同步时写入）
- `private` 部分：调 Gitea API `GET /user/repos` 反查当前 user 可访问的 repo 列表（5 分钟 Redis 缓存）

**新字段（仅 capability_registries 表，1 个）**：

```sql
ALTER TABLE capability_registries
  ADD COLUMN gitea_visibility VARCHAR(16);  -- public | private，sync worker 同步时写入
```

`capability_items` 表**不**加 visibility 字段——继承所在 registry 的 visibility。

**Pack 内 plugin 私有化不支持**：pack repo 整体 public/private，pack 内某 plugin 不能单独私有。需要"部分 plugin 私有"时拆成两个 pack repo。

**业务线 internal 边界**（§9.10 E4 简化方案）：

- 业务线层级仅在 costrict-web 发现层过滤，**不**通过 Gitea team 实现
- private repo 仅靠 per-user collaborator（admin 手动邀请，离职自动清理）
- 业务线外用户访问 → Gitea private repo 天然屏蔽（无 collaborator 即 404）
- 假设条件：private capability 占比 < 10%；未来占比上升可平滑切到方案 3（dept → Gitea team sync），不影响现有 repo 命名

**公有能力贡献流程**：

- 用户/业务线提 PR 到 `costrict/<slug>` 或 `costrict/curated-seed`（精选 mono-repo）；平台配置走 `costrict-config/platform-config` PR（admin 审核 yaml 变更）
- admin 审核 → merge → repo 强制 public
- **不允许**用户个人 namespace 的 repo 直接 public——必须先迁移到 `costrict/` org
...
## 9. 认证集成（costrict-web 用户中心 + fork Gitea JWT 中间件）
...
- **Gitea（fork）**：在 `routers/common/auth.go` 链最前插入 JWT 中间件（~250 行 fork），验证 costrict-web 签发的 JWT + auto-provisioning（首次见到新 user 自动创建 Gitea user）；不暴露用户管理 UI（headless）
- **系统服务账号 `costrict-system`**：单一 site-level user，签发**单一 admin PAT**；仅用于真正"无用户上下文 + 必须跨用户身份"的 2 个场景（见 §9.7）；AI agent / csc 等用户侧行为**不走 bot**，统一按用户自己 PAT（见 §9.8.3）
   ...
   └─► 选登录方式（密码 / GitHub OAuth / 短信 / ...）
       └─► Casdoor 处理多源登录 UI（仅登录 UI 提供者）
           └─► 回调 costrict-web ─► costrict-web 自签 JWT（含 user_id / preferred_username / email / groups）
               └─► 写入 user_gitea_binding（gitea_username = preferred_username）
...
用户访问 Gitea
   └─► 携带 costrict-web JWT（Authorization: Bearer ...）
       └─► Gitea fork JWT 中间件：
           ...
           ├─ 查 Gitea user：
           │   ├─ 存在 → 注入 session
           │   └─ 不存在 → auto-provision（创建 Gitea user，username = preferred_username）
...
- costrict-web `users.id` → Gitea `user.email` 字段（写入 `external_id` 形式，便于反查）；Gitea `user.name` = costrict-web `preferred_username`
...
|---|---|---|---|
| 注册 | 校验 username 唯一 + 写 `users` + 签 JWT | 无（lazy 创建） | 无 |
| 首次访问 Gitea | 无 | JWT 中间件 auto-provision | 无 |
...
- auto-provisioning：调 Gitea internal `models.CreateUser`（不走 admin API 权限校验，避免中间件持 admin token）
...
### 9.7 Gitea site-admin token（单一 `costrict-system` 账号）
...
| **2** | **用户生命周期级联** | `user.updated` / `user.deleted` / `user.disabled` webhook | `PATCH /admin/users/{name}` + repo ownership transfer + collaborator 删除 | 这些是 Gitea admin-only API，JWT 中间件走不通 |

**marketplace mirror 同步不需要 admin token**：走 Gitea 内置 mirror pull 功能（§7.6 方式 A），Gitea 自己周期 `git fetch upstream`，不调 REST API。
...
#### 9.7.5 fork 中间件不持 admin token

auto-provisioning 走 Gitea internal API（`models.CreateUser`），与 admin token 解耦；中间件仅做 JWT 验证 + 用户创建，不持有任何特权凭据。
...
| **用户 PAT（csc / AI agent 代用户）** | clone private repo | fine-grained PAT `read:repository` | Gitea PAT（§9.8.3） |
| **用户 PAT（csc / AI agent 代用户）** | push main / feature branch | fine-grained PAT `write:repository` | Gitea PAT（§9.8.3）；允许直接 push main（§9.8.2 已放开） |
| **`costrict-system` admin PAT** | sync worker 跨 owner 拉 private repo | admin scope | 仅 §9.7.2 列出的 2 个场景，禁止他用 |
| 用户/agent | push 到 main / master | **允许**（直接 push） | Gitea PAT + 健康度治理兜底（§8.4）+ 硬配额（§9.8.5） |
| CI（Gitea Actions） | push 后跑校验，标 commit status | 内置 `${{ secrets.GITHUB_TOKEN }}` | 非阻塞，仅信息展示 |
| csc 设备端 | （不走 git，走 HTTP 代理） | N/A | 见附录 C.2.3 |

#### 9.8.2 Branch Protection（简化：仅防历史覆写）

> **决策**：V3 是认证用户内部协作平台，依赖健康度治理（§8.4）+ webhook 触发 sync + security scan + 硬配额（§9.8.5）+ git log 完整审计兜底，**不强制 PR + review + CI 通过门**。保留 git 原生直接 push 体验，与 V2 编辑 UX 一致，对 AI agent 操作最友好。
...
| 风险 | 兜底 |
|---|---|
| AI agent 误推送 | §8.4 健康度自动标 polluted + admin 标 archived |
| 用户 typo / 误操作 | `git revert` 30s 恢复；commit 历史保留完整 |
| 恶意推送 | commit author 归因 + Gitea audit log + admin 标 archived + 用户 PAT 撤销 |
| 大量垃圾推送 | §9.8.5 fork pre-receive hook 硬配额（10MB / 50MB）拦截 |
| 上游 mirror 关键文件消失 | §8.4 polluted 自动检测 + 卡片灰显 |
...
#### 9.8.3 用户 PAT 设计（fine-grained，D 方案 C 通道）
...
#### 9.8.5 硬配额拦截（fork 全局 pre-receive hook）
...
#### 9.9.4 branch-protection.yaml 示例（v3 简化方案）

```yaml
# .gitea/branch-protection.yaml
# v3 简化：仅保留禁 force push + 禁 delete main；
# 不要求 PR / reviewer approve / CI 必过（详见 §9.8.2 决策）
default_rules:
  - pattern: main
...
### 9.10 业务线层级与 Gitea 关系（E4 简化方案）

> **决策**：Gitea 不掺业务组织概念，业务线层级仅用于 costrict-web 发现层；Gitea 仅承担 git 协议 + repo visibility + per-user collaborator。dept-sync 服务保留真相源地位，costrict-web 仅做 webhook 接收 + Redis cache。
...
| **costrict-web** | 发现层 API 按 `user.dept_id` 过滤；private repo admin 邀请 UI；离职自动化清理 |
...
[costrict-web]
   ...
   ├─ 发现层 API：按 user.dept_id 过滤可见 capability
...
| 业务线 → capability 关系 | costrict-web `capability_items.business_line` 字段（已有），仅用于发现层过滤 |
...
CREATE TABLE user_gitea_binding (
    ...
    gitea_uid        INT       NOT NULL UNIQUE,    -- Gitea user.id
    gitea_username   VARCHAR(64) NOT NULL,         -- Gitea user.name（可变，username 改名后更新）
...
#### `webhook_subscriptions`（通用 webhook 订阅表，承载 §9.5 多目标广播）
...
### Stage 0：fork Gitea + JWT 中间件 + 全局 pre-receive hook + costrict-web 用户中心（约 3-4 周）

> 前置阶段，所有后续阶段依赖此 baseline。

- [ ] fork Gitea（基于稳定 LTS 版本，建议 1.21+），fork 仓库 `costrict-plugins-repo/gitea` 维护
- [ ] **JWT 中间件**（~250 行）：
  - 实现 `routers/common/auth_jwt.go`（~200 行）：JWKS cache（5min TTL，从 costrict-web `/.well-known/jwks.json` 拉）+ RS256 验证 + clock skew ±60s + auto-provisioning（调 Gitea internal `models.CreateUser`，不持 admin token）
  - 修改 `routers/common/auth.go` + `routers/web/routes.go`（~50 行）：注册中间件到 auth 链最前
  - 失败降级：JWKS 拉不到 → 用 5min 前旧 key；JWT 无效 → 401
- [ ] **全局 pre-receive hook**（~150 行）：
  - 修改 `modules/gitrepo/hooks.go` `CreateDelegateHooks`（~20 行）：加入系统级 pre-receive hook 路径 fallback
  - 新增 `modules/git/preceive_global.go`（~80 行）：实现单文件大小 + repo 总配额检查；commit message 不检查
  - 新增 `modules/setting/quota.go`（~20 行）：加载 app.ini 中的 quota 默认配置
  - 新增 `routers/internal/quota.go`（~30 行）：`POST /api/internal/quota-cache-invalidate` endpoint，costrict-web sync worker 主动失效 cache 用
  - PostgreSQL connection pool 直连共享 DB 的 `gitea_ext.quota_rules` 表（5min TTL cache）
- [ ] costrict-web 端：
  - 实现 JWT 自签（RS256，私钥存 secret store）
  - 暴露 `/.well-known/jwks.json` endpoint
  - 新增表（见 §10.4）：`user_gitea_binding` / `user_profile` / `webhook_subscriptions` / `webhook_deliveries` / `gitea_admin_audit_log` / `gitea_ext.quota_rules` / `gitea_ext.config_versions`
  - 实现通用 webhook 广播 worker（监听 `user.updated` / `user.disabled` / `user.deleted` 事件 + 6 次重试 + 死信队列）
  - 实现 sync worker：监听 `user.updated` → 调 Gitea admin API `PATCH /admin/users/{old}` 改 username
  - 实现 `GiteaConfigSyncWorker`（监听 `costrict-config/platform-config` push 事件，diff `.gitea/*.yaml` 变更，分别调 Gitea API 应用 + 写 `gitea_ext.quota_rules` + 调 fork 内部 endpoint 失效 cache）
  - 持有 Gitea site-admin token（90 天轮换 + 审计日志）
- [ ] `costrict-config/platform-config` repo 初始化（创建以下文件并 PR）：
  - `.gitea/branch-protection.yaml`（默认 + overrides，见 §9.9.4）
  - `.gitea/quota.yaml`（owner 默认 + per-repo 覆盖，见 §9.9.3）
  - `.gitea/teams.yaml` / `.gitea/webhooks.yaml` / `.gitea/labels.yaml`
  - `ISSUE_TEMPLATE/*.md` / `PULL_REQUEST_TEMPLATE.md`
  - `README.md`（说明每个 yaml 的 schema 与编辑流程）
- [ ] `costrict/curated-seed` repo 初始化（可选，留空 README + 目录骨架，等后续填充精选能力项）
- [ ] Casdoor 退化为多登录源 UI 提供者（保留现有 `UserAuthIdentity` 表，costrict-web 自签 JWT）
- [ ] CI / 测试：
  - fork Gitea 单测：中间件正确解析 JWT / auto-provisioning 幂等 / JWKS cache 命中率 / 全局 pre-receive hook 拦截大文件 / 配额超额 / cache 失效
  - 集成测试：username 变更后 Gitea username 同步生效 / push 大文件被拒 / 配额变更实时生效 / platform-config yaml 变更触发 batch sync
- [ ] 部署：fork Gitea 镜像（替换官方 Gitea 镜像），灰度环境联调
- [ ] **维护计划**：每季度 Gitea upstream release 跟进 rebase；fork 改动限定在认证 + hook 模块（~400 行）便于 rebase

### Stage 1：基础设施（约 1 周）

- [ ] 部署 Gitea（与 costrict-web 共享 PostgreSQL + Redis，附件走 MinIO）
- [ ] Gitea fork 部署 + JWT 中间件配置（JWKS endpoint = `https://costrict-web/.well-known/jwks.json`，§9.6）；**不再配 OAuth2 Source**（v3 走 JWT 中间件而非 OIDC）
- [ ] 验证用户 SSO + fork JWT 中间件 auto-provisioning 流程
- [ ] 验证 `costrict-system` admin PAT 流程（capability 索引同步 + 用户生命周期级联）
- [ ] 验证用户 PAT 自助生成流程（`/settings/git-tokens`）+ csc 端 token 配置
- [ ] 在 Gitea 建立 4 类固定 org：`costrict-config`（配置中心）/ `costrict`（官方能力项）/ `costrict-plugins`（pack）/ `costrict-mirror`（镜像）；精选 mono-repo `costrict/curated-seed` 与配置中心 `costrict-config/platform-config` 在 Stage 0 末初始化（用户 namespace `u-<username>/` 由 fork JWT 中间件 auto-provision 时按需创建，不预先批量建）
- [ ] 写 frontmatter schema 与 AI 贡献指南（放 `costrict/curated-seed` 仓库 README）；GitOps 配置编辑指南放 `costrict-config/platform-config` 仓库 README
- [ ] **注册 system webhook**：在 Gitea admin 面板配置 system-level webhook，事件类型 `push` + `pull_request`，目标 `POST /api/internal/git-sync` 和 `POST /api/internal/capability-check`（含签名校验）
- [ ] **dept-sync 集成**（E4 简化方案）：
  - dept-sync 服务加 webhook 推送接口（`dept.updated` / `dept.member_changed` 事件，HMAC 签名）
  - costrict-web 新增 `DeptSyncCacheWorker`：接收 webhook → 写 Redis（5min TTL，key = `dept_tree` / `dept:{id}:members`）
  - costrict-web 现有 `/admin/departments/tree` API 改为读 Redis cache（dept-sync 故障时降级 cache 兜底）
  - costrict-web 新增 private repo admin 邀请 UI（`/admin/private-repos` 页面，调 Gitea collaborator API）
  - costrict-web 新增离职清理 worker：监听 `user.disabled` / `user.deleted` → 调 Gitea admin API 移除该 user 所有 collaborator + 禁用 Gitea 账号

### Stage 2：仓库分类与数据导出（约 2 周）

- [ ] 给现有 `capability_items` 分类：
  - 上游 GitHub 来源 → 标记为 `mirror` kind，建立 mirror repo
  - plugin pack 来源 → 标记为 `pack` kind，建立 pack repo
  - 用户独立创建 → 标记为 `standalone` kind，一 item 一 repo
  - 官方精选 → 标记为 `seed` kind（可选）
- [ ] 对每类 repo 写迁移脚本：
...
- [ ] DB 加新字段（`source_repo_url` 等），写回 baseline 值
- [ ] **冻结**现有 catalog ingest 链路（V2 `migrate ingest-upstream` + `CatalogIngestService` 代码不删，admin 不主动跑；仅作 V3 故障时的应急 fallback）

### Stage 3：webhook 接通与双写灰度（约 2-3 周）

- [ ] 实现新 sync worker：监听 Gitea webhook → 拉取 → diff → 更新 DB（按 kind 分发）
- [ ] 实现新 endpoint：`POST /api/internal/git-sync`（接受 Gitea webhook）
- [ ] **实现 capability-check worker**：监听 PR webhook → 拉文件结构 → 启发式识别 + schema 校验 → 写 `health_issues` → 调 Gitea API 评论 PR summary
- [ ] **实现新 endpoint**：`POST /api/internal/capability-check`（接受 Gitea PR webhook）
- [ ] **安全扫描迁移（见 §8.5）**：
  - `security_scans` 表加 `git_sha VARCHAR(40)` + `trigger_type VARCHAR(32)` + `pr_number INT` 字段（双写兼容期）
  - `scan_job_service.Enqueue` 改造：短路键 `CurrentRevision` → `git_sha`；签名扩展加 `git_sha` + `trigger_type` + `pr_number` 入参
  - scan 调用点迁移：`catalog_ingest_service.go:1024,1130` + `sync_service.go:421,491` → 新 sync worker（监听 Gitea webhook）
  - PR 触发分支：capability-check worker 在 PR webhook 时调 Enqueue(`trigger_type=pr-check`)，结果写 `security_scans` 但不覆盖主表
  - PR 评论逻辑：调 Gitea API 写 health + security summary（`medium`+ 默认折叠）
  - post-merge 触发：sync worker → Enqueue(`trigger_type=git-push`) → 覆盖 `capability_items.security_status`
  - Plugin 跳过逻辑保留（`scan_service.go:252-254` 不动）
  - manifest 透传：`health.identification` + `health.security` 双字段
- [ ] 灰度策略：
  - 先开放**只读**：所有 repo 变更 → DB sync（V2 ingest-upstream 应急 fallback 保留但冻结）
  - 再开放**写入**：用户/AI **默认直推 main**（§9.8.2 简化方案）；**可选走 PR** 用于公有能力贡献 / 重要变更评审（§8.1.1）；REST API `/api/items POST/PUT/DELETE` 逐步下线
  - 业务字段（favorite/install/security）API 保留

### Stage 4：mirror 接入自动化（约 1 周）

- [ ] 在 Gitea 配置所有 mirror repo 的 pull 频率（统一 24h）
- [ ] server 端记录每个 mirror 的 `mirror_of` 字段
- [ ] 上游 repo 删除检测：mirror pull 失败 → 标记 `archived`

### Stage 5：下线旧通道与清理（约 1 周）

> **启动条件**：V3 sync worker 稳定运行 ≥ 2 周无 P0 故障；期间未触发过 V2 应急 fallback（即 V3 自身足够可靠）。

- [ ] 下线 `migrate ingest-upstream` 命令
- [ ] 下线 `services.CatalogIngestService`
...
- [ ] 文档更新：`CATALOG_INGEST.md` 改为 `CAPABILITY_GIT_SYNC.md`
- [ ] 监控仪表盘：sync 延迟、webhook 失败率、PR 待审时长、DB 行数趋势
...
| 13 | 公私能力 visibility 策略 | **纯 Gitea visibility 透传**：取值仅 `public`/`private`（对齐 Gitea 原生，不引入 `internal`/`hidden`）；业务线层级仅用于 costrict-web 发现层过滤（§9.10 E4 简化方案，**不**通过 Gitea team 实现）；pack 内 plugin 不支持单独私有（拆 pack repo 解决）；公有能力贡献走 PR 到 `costrict/` + admin 审核；用户自建走 `u-<username>/` namespace（默认 private，marketplace 默认过滤）；server 用 `costrict-system` admin PAT sync，发现层 API 双层过滤（public 直接 + private 调 Gitea 反查） |
| 14 | 用户中心主权归属 | **costrict-web 承担用户中心**（username / email / 密码 / 业务字段主权）；Casdoor 退化为多登录源 UI 提供者；Gitea fork 加 JWT 中间件实现 user 自动同步。否决"Gitea 做用户中心 + HA"方向（见 §14.6） |
| 15 | Gitea fork 改动范围 | **最小化 fork**：JWT 中间件 + auth 链注册（~250 行）+ 全局 pre-receive hook（~150 行）= **总计 ~400 行**；不动 UI / cron / mirror / webhook 投递 / Actions；fork 维护成本可控，每季度 rebase upstream；详细见 §11 Stage 0 |
...
| 19 | Git 操作权限管理 | **保留 Gitea 原生 git 协议接口**（HTTP + SSH），用户/AI agent 直接 `git clone / push`；**branch protection 仅保留禁 force push + 禁 delete main**（不强制 PR / review / CI 通过门，详见 §9.8.2 简化方案）；AI agent / csc 用 fine-grained 用户 PAT（`read:repository` + `write:repository`，限 owner `costrict` / `costrict-plugins`，90 天轮换，明确禁止共享，D 方案 C 通道）；用户用 SSH key 优先（与 GitHub 体验一致）；deploy key 仅 CI/外部系统使用；误推送兜底靠 §8.4 健康度治理 + git revert + admin 标 archived + 硬配额（§9.8.5）|
...
| 24 | Branch Protection 简化（直接 push 模式） | **放开强制 PR / review / CI 阻塞门**：V3 是认证用户内部协作平台，main 分支仅保留禁 force push + 禁 delete（防历史覆写）；用户/AI agent 可直接 push main，体验与 V2 一致；不要求 PR、不要求 reviewer approve、不要求 CI 必过（capability-check CI 仍跑，仅作 commit status badge，非阻塞）；误推送兜底链路：§8.4 健康度自动 polluted + git revert 30s 恢复 + admin 标 archived + §9.8.5 硬配额拦截大文件 + commit author 归因 + Gitea audit log；适用前提：全部认证用户 + 团队 < 10 人；未来如需切回强制 PR 仅改 Gitea 配置无架构改动；实现工时 0 天 |

---

## 14. 替代方案

### 14.1 已否决：全 mono-repo

- 丢失上游原生形态，与 mirror / pack 来源冲突
- 跨 org 权限隔离难
- "唯一仓库地址"语义弱化

### 14.2 已否决：全一 item 一 repo（强解读）

- Plugin pack 被强行拆解，失去上游一致性
- Mirror 失去原貌
- 仓库数爆炸，运维负担重

### 14.3 已否决：完全去掉 DB，纯 Git

- 业务字段（计数、扫描结果、用户关系）无处安放
- 跨 repo 搜索能力差

### 14.4 已否决：用 Gitea 替代 Casdoor 当 IdP

- Gitea 多登录源管理 UI 弱于 Casdoor
- 见 `MULTI_PROVIDER_ACCOUNT_BINDING_DESIGN.md`、`USER_TABLE_DESIGN.md` 等历史决策

### 14.5 已否决：用 PostgreSQL 的 git-like 扩展（如 Dolt）

- 缺乏 AI 工具链支持
- 引入新组件运维成本高

### 14.6 已否决：Gitea 做用户中心 + Gitea HA（v3 评审否决）
...
保留方案（已采纳）：costrict-web 用户中心 + fork Gitea JWT 中间件 + 全局 pre-receive hook（~400 行最小化 fork），见 §9。

---

## 15. 后续工作（Out of Scope）
...
- Gitea JWT 中间件 fork 改动：见 §9.6（替代 v1 的 OAuth2 Source 方案，v3 不再使用 OAuth2 Source）
...
| 模块 | V2 行为 | V3 改造 |
|---|---|---|
| `GET /api/items` 列表 API | DB content 字段直读 | DB 改为 metadata + business fields 索引；content 不再直读（参考字段） |
...
| `capability_items` 表 | 含 SourceSHA / CatalogEntryDir / CurrentRevision / Content | **新增**：source_repo_url / source_repo_path / source_repo_ref / source_repo_kind / capability_type / git_sha / git_last_synced_at / mirror_of / identification_status / health_issues / last_checked_at / last_clean_at；**废弃**：SourceSHA / CatalogEntryDir / CurrentRevision / SourceType（双写兼容期后删） |
...
| 模块 | 改造内容 |
|---|---|
| `routers/common/auth_jwt.go`（新增） | JWT 中间件 ~200 行：JWKS cache（5min TTL）+ RS256 验证 + auto-provisioning |
...
|---|---|---|---|
| content 下发流程 | DB 直读 | HTTP 代理 + Gitea permission API 反查（A2） | §13 第 18 行 |
| 用户中心 | Casdoor + costrict-web 双层 | costrict-web 主权 + Casdoor 退化 + fork 中间件 | §13 第 14-17 行 |
...
| fork Gitea 改动 | - | ~400 行（auth_jwt.go + 全局 pre-receive hook + 注册） | §13 第 15 行 / 本附录 C.2.4 |
