...
func buildVisibleRegistryIDs(db *gorm.DB, userID string) []string {
	...
	db.Model(&models.SkillRegistry{}).Where("visibility = 'public'").Pluck("id", &publicIDs)
	...
	if userID != "" {
		...
		if len(orgIDs) > 0 {
			var orgRegs []string
			db.Model(&models.SkillRegistry{}).Where("org_id IN ? AND visibility = 'org'", orgIDs).Pluck("id", &orgRegs)
			ids = append(ids, orgRegs...)
		}

		var personalRegs []string
		db.Model(&models.SkillRegistry{}).Where("owner_id = ?", userID).Pluck("id", &personalRegs)
...
func ListAllItems(c *gin.Context) {
	...
	if len(registryIDs) == 0 {
		c.JSON(http.StatusOK, gin.H{"items": []models.SkillItem{}, "total": 0})
		return
	}
	...
	var total int64
	query.Model(&models.SkillItem{}).Count(&total)
