...
//
// v2 widened pluginFlattenPlanDigest from "identity + intended end state" to
// "every field except the two that mutate during apply". v1 digests are not
// comparable and v1 runs/artifacts are refused rather than reinterpreted.
const pluginFlattenSchemaVersion = 2
...
const (
	...
	// flattenRowApplied means THIS run's compare-and-set claimed the row.
	flattenRowApplied = "applied"
	// flattenRowAlreadyAtTarget means the predicate matched nothing and the live
	// row already held the intended end state. That is somebody else's write —
	// the plan's data write and its row marker commit in ONE transaction, so a
	// row this run changed can never be missing its marker. Recording it as
	// `applied` would make rollback revert a state the migration never caused.
	flattenRowAlreadyAtTarget = "already_at_target"
...
type pluginFlattenOptions struct {
	...
	ArtifactPath string
	BatchSize    int
	// BatchSizeSet distinguishes "the operator asked for this batch size" from
	// "nobody said, so the default is in the field". Without it, apply could not
	// tell whether to honour --batch-size or the size frozen into the run at plan
	// time — and it silently ignored the flag, so `--batch-size=500` on 6738 rows
	// ran 34 batches of 200 while the operator believed they had asked for 14.
	BatchSizeSet bool
...
type flattenCandidate struct {
	...
	// ForkSourceLinked: the row this was forked from still carries a parent link.
	ForkSourceLinked bool
	// ForkSourceRetired: the row this was forked from no longer carries a parent
	// link because an EARLIER flatten run retired it as package-derived content.
	//
	// Without this, the fork rule would be destroyed by the first apply: it asks
	// whether the fork source is a package child, apply unlinks package children,
	// so every fork left over for a second round would flip from derived_fork to
	// ambiguous and be stuck needing a human forever — while the runbook tells
	// operators to re-plan for exactly that second round. The migration's own
	// durable row table is the evidence that survives its own writes.
	ForkSourceRetired bool
	FavoriteCount     int
...
func classifyFlattenCandidate(c flattenCandidate) flattenVerdict {
	...
	case "fork":
		...
		if !c.ForkSourceLinked && !c.ForkSourceRetired {
...
func loadFlattenCandidates(tx *gorm.DB) ([]flattenCandidate, error) {
	...
	// ...or while an earlier run of THIS migration is on record as having
	// retired it as package-derived content. The live column above cannot answer
	// that, because unlinking the source is precisely what apply does — so
	// without this lookup the second plan of a two-round cleanup would reclassify
	// every remaining fork as ambiguous and strand it, and the runbook's own
	// "re-plan a new run to finish up" instruction would be the thing that broke
	// it. `unlink_only` (independent) rows are excluded on purpose: those were
	// never package children, so their unlinking proves nothing about a fork.
	forkSourceRetired := map[string]bool{}
	if err := scanInChunks(tx, forkSourceIDs, func(chunk []string) error {
		var got []string
		if err := tx.Table("plugin_flatten_migration_rows").
			Where("item_id IN ? AND row_state = ? AND classification IN ?",
				chunk, flattenRowApplied,
				[]string{flattenClassDerivedCatalog, flattenClassDerivedArchive, flattenClassDerivedFork}).
			Distinct().Pluck("item_id::text", &got).Error; err != nil {
			return err
		}
		for _, id := range got {
			forkSourceRetired[id] = true
		}
		return nil
	}); err != nil {
		return nil, err
	...
	for _, it := range items {
		...
		linked, retired := false, false
		if it.ForkedFromItemID != nil {
			linked = forkSourceLinked[*it.ForkedFromItemID]
			retired = forkSourceRetired[*it.ForkedFromItemID]
		}
