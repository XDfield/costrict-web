...
| 版本 | v2.16（新增 `costrict-workflow/` org 与 §17 Workflow 仓库管理与协作；workflow repo path 由 server 纯函数从 `(workflow_def_slug, instance_id)` 推导；节点级 PR 审计 + 实例级生命周期管理） |
|---|---|
| 创建日期 | 2026-07-09 |
| 依据 | [`CAPABILITY_GIT_REGISTRY_PROPOSAL_V4.md`](./CAPABILITY_GIT_REGISTRY_PROPOSAL_V4.md) |
...
| `costrict-kb` | 知识库 repo（与代码 repo 一一对应；§16） | **private**（强制；`members_can_create_repos=false`，所有 repo 由 server admin PAT 在 `POST /api/kb/ensure` 中创建） |
| `costrict-workflow` | workflow 实例交付物 repo（与实例一一对应；§17） | **private**（强制；`members_can_create_repos=false`，所有 repo 由 server admin PAT 在 `POST /api/workflow/init` 中创建；节点级 PR 审计 + 实例级生命周期管理） |
...
### 4.6 KB repo 命名（v2.15）

KB repo 命名不依赖用户输入 slug，由 server 端纯函数从 `code_repo_url` **确定性地推导**，输出固定格式：

```
costrict-kb/<host>__<escaped_segments_joined>
```

详细算法、字符转义规则、长度截断策略、测试用例见独立 spec：[`KB_REPO_PATH_ALGORITHM.md`](./KB_REPO_PATH_ALGORITHM.md)。

示例：

| 代码 repo URL | KB repo path |
|---|---|
| `https://github.com/ownerA/proj.git` | `costrict-kb/github.com__ownera__proj` |
| `https://gitlab.com/Group.Foo/bar-baz.git` | `costrict-kb/gitlab.com__group.foo__bar-baz` |
| `https://gitea.costrict.local/team-x/internal-svc` | `costrict-kb/gitea.costrict.local__team-x__internal-svc` |

**强约束**：
- 算法**唯一来源是 server**（csc 不内置副本，路径仅来自 `POST /api/kb/ensure` 响应）
- 输出路径**永远落在 `costrict-kb/` org 下**，不允许用户指定其他 namespace
- 算法版本当前为 v1，**不加路径前缀版本号**（Gitea 路径只支持 2 层 `<owner>/<repo>`）
- 用户 PAT **无权访问 `costrict-kb/` org 的 repo 创建**——所有创建走 server admin PAT

### 4.7 Workflow repo 命名（v2.16）

Workflow repo 命名不依赖用户输入 slug，由 server 端纯函数从 `(workflow_def_slug, instance_id)` **确定性地推导**，输出固定格式：

```
costrict-workflow/<def_slug_escaped>__<instance_short_id_8chars>
```

详细算法、字符转义规则、长度截断策略、测试用例见独立 spec：[`WORKFLOW_REPO_PATH_ALGORITHM.md`](./WORKFLOW_REPO_PATH_ALGORITHM.md)。

示例：

| workflow_def_slug | instance_id | wf_repo_path |
|---|---|---|
| `bug-fix-flow` | `f3a8b2c1-9d7e-4a2b-8e1f-1234567890ab` | `costrict-workflow/bug-fix-flow__f3a8b2c1` |
| `compliance-check` | `a9e7d4f2-1234-5678-9abc-def012345678` | `costrict-workflow/compliance-check__a9e7d4f2` |
| `release-pipeline` | `00000000-0000-0000-0000-000000000001` | `costrict-workflow/release-pipeline__00000000` |

**强约束**：
- 算法**唯一来源是 server**（csc 不内置副本，路径仅来自 `POST /api/workflow/init` 响应）
- 输出路径**永远落在 `costrict-workflow/` org 下**，不允许用户指定其他 namespace
- 算法版本当前为 v1，**不加路径前缀版本号**（Gitea 路径只支持 2 层 `<owner>/<repo>`）
- 用户 PAT **无权访问 `costrict-workflow/` org 的 repo 创建**——所有创建走 server admin PAT
- `instance_id` 由平台 workflow 编排器（独立服务）在实例启动时分配全 tenant 唯一 UUID，server / csc 不生成
...
| 知识库（KB）repo | `costrict-kb/<host>__<escaped_segments>`（v2.15；路径由 server 纯函数从 `code_repo_url` 推导，详见 [`KB_REPO_PATH_ALGORITHM.md`](./KB_REPO_PATH_ALGORITHM.md)） | **server admin PAT**（`POST /api/kb/ensure` 幂等接口；首个调用者自动成为 repo owner，详见 §16） | **private**（强制；`members_can_create_repos=false`） |
| Workflow 实例交付物 repo | `costrict-workflow/<def_slug>__<instance_short_id>`（v2.16；路径由 server 纯函数从 `(workflow_def_slug, instance_id)` 推导，详见 [`WORKFLOW_REPO_PATH_ALGORITHM.md`](./WORKFLOW_REPO_PATH_ALGORITHM.md)） | **server admin PAT**（`POST /api/workflow/init` 接口；首个调用者自动成为 repo owner，节点级 PR 审计 + 实例级生命周期，详见 §17） | **private**（强制；`members_can_create_repos=false`） |
...
- **`costrict-kb/` org 的 repo 创建权独占给 server admin PAT**（§16）；用户 PAT 仅可对**自己有 collaborator 关系的 KB repo**做 push / clone / PR；用户 PAT **不允许任何 scope 包含 `costrict-kb` owner 的 repo:create 权限**——避免用户绕过 `POST /api/kb/ensure` 直接建 repo 导致 owner 授权缺失
- **`costrict-workflow/` org 的 repo 创建权独占给 server admin PAT**（§17）；用户 PAT 仅可对**自己有 collaborator 关系的 workflow repo**做 push / clone / PR；用户 PAT **不允许任何 scope 包含 `costrict-workflow` owner 的 repo:create 权限**——避免用户绕过 `POST /api/workflow/init` 直接建 repo 导致 owner 授权缺失与 definition snapshot 缺失
...
| KB | 用户 PAT 在 `costrict-kb/` org 内直接建 repo（绕过 `POST /api/kb/ensure`） | 导致首个 push 用户未成为 owner，后续协作授权链断裂 |
| KB | csc 内置算法副本计算 KB path（与 server 漂移） | 双端不一致，同一 URL 算出不同 path |
| KB | server 维护 `kb_repos` / `codebase_kbs` 等绑定表 | 纯函数已能唯一映射，多维护一份状态反成漂移源 |
| KB | 在 server 内编排 KB 协作流程（issue / 授权 / 转让） | 应回归 git 原生协作 + Gitea API 透传 |
| Workflow | 用户 PAT 在 `costrict-workflow/` org 内直接建 repo（绕过 `POST /api/workflow/init`） | 导致 definition.snapshot.yaml 缺失，节点级 PR 审计链断；owner 授权缺失 |
| Workflow | csc 内置算法副本计算 wf path（与 server 漂移） | 双端不一致，同一 (def_slug, instance_id) 算出不同 path |
...
| Workflow | 跨实例共享同一 repo（多实例 branch 模式） | 权限粒度只能到类型级、branch 不能删、PR 流程嵌套，详见 §17.1 设计对比 |
...
1. **代码 repo → KB repo 一一对应**：本地代码 repo 用 csc 生成 KB 文档，推送到 git server 上的对应 KB repo；同一代码 repo 的不同用户共享同一 KB repo
2. **路径推导纯函数化**：KB repo path 由 server 端纯函数从 `code_repo_url` 推导（详见 [`KB_REPO_PATH_ALGORITHM.md`](./KB_REPO_PATH_ALGORITHM.md)），**不维护绑定关系表**
3. **server 仅预创建**：`POST /api/kb/ensure` 幂等接口承担"判存在 + 首次创建 + owner 授权"三件事，**其他全部回归 git 原生协作**（push / pull / branch / collaborator / PR / issue 均走 Gitea 原生 API）
4. **无 server 状态**：server 不持久化 KB 元数据，不维护 `kb_repos` 表，不算算法缓存——每次 ensure 实时计算 + 实时查 Gitea
5. **无权场景透明化**：调用者对已有 KB repo 无 collaborator 关系时，server 在 ensure 响应中返回 owner 信息（用户名 + 显示名），由调用者线下联系 owner 索权

### 16.2 org 配置：`costrict-kb/`

| 字段 | 值 | 说明 |
|---|---|---|
| org name | `costrict-kb` | 全 tenant 共享固定名（与 `costrict-official` / `costrict-template` / `costrict-config` / `costrict-mirror` 同位） |
| visibility | **private**（强制） | KB 内容仅协作团队可见，不暴露给 tenant 内其他用户 |
| `members_can_create_repos` | **false** | 用户不能在 UI 直接建 repo；所有 repo 创建走 server admin PAT |
| team 结构 | **无 team** | 不维护 `kb-creators` 等团队（人员流动频繁，团队维护成本高）；权限模型直接落到 collaborator |
| 成员列表 | **空**（org 层面无成员） | 所有权限通过 repo 级 collaborator 表达 |
| SSO 自动加入 | **否** | 用户首次 ensure 时由 server admin PAT 临时加为 collaborator（仅对本次创建的 repo 生效），不影响其他 repo |
| webhook | 复用 tenant `costrict-config/platform-config` 中 `webhook_secret_ref` | KB repo push webhook 投递到独立的 KB 生成服务（不在本规范范围） |

### 16.3 server API：`POST /api/kb/ensure`
...
```json
{
  ...
  "role": "owner",           // "owner" | "write" | "none"
  "created": false,           // 本次调用是否新建了 repo
  "owner": {                  // role=none 时必填，便于线下联系
    "username": "alice",
    "display_name": "Alice"
  },
  "hint": ""                  // 人类可读的下一步操作提示
...
| 场景 | server 行为 | role | created | owner | hint |
|---|---|---|---|---|---|
| **不存在**（首次 ensure） | ① 用 `costrict-system` admin PAT 调 `POST /admin/users/costrict-kb/repos` 建 repo ② 调 `POST /repos/.../collaborators/<caller>` 把调用者加为 owner permission ③ 调 `POST /repos/.../branch_protections` 配置 main 分支保护（禁 force push / delete） | `owner` | true | 调用者自身 | 空 |
| **存在 + 调用者可访问** | 调 `GET /repos/.../collaborators/<caller>/permission` 反查权限 | `owner` / `write` | false | 实际 owner | 空 |
| **存在 + 调用者无权** | **不创建、不改权限**；调 `GET /repos/...` 取 owner username（用 admin PAT）→ 调 `GET /users/<owner>` 取 display_name | `none` | false | 实际 owner | `"KB repo 已由 <owner.display_name>(@<owner.username>) 创建，请通过站内消息 / IM / email 联系其授予 collaborator write 权限后重试"` |

#### 16.3.4 错误码

| HTTP | 触发条件 | 响应体 |
|---|---|---|
| 400 | `code_repo_url` 缺失 / 非 http(s) scheme / 解析失败（详见算法 spec §4.4） | `{ "error": "invalid_code_repo_url", "detail": "..." }` |
| 401 | 调用者未认证（JWT 缺失 / 过期） | `{ "error": "unauthorized" }` |
| 403 | JWT tenant_id 与本实例归属 tenant 不一致（§1.5.2） | `{ "error": "tenant_mismatch" }` |
| 500 | admin PAT 调 Gitea 失败（建 repo / 加 collaborator / 配保护） | `{ "error": "gitea_api_failed", "detail": "..." }` |

#### 16.3.5 幂等保证

- 同一调用者重复 ensure 同一 `code_repo_url`：第二次进入"存在 + 可访问"分支，返回 `role=owner / created=false`
- 不同调用者先后 ensure 同一 URL：第二个进入"存在 + 无权"分支，返回 owner 信息
...
2. csc 调用 POST /api/kb/ensure
   ├─ 携带调用者 JWT
   └─ body: { code_repo_url }

3. 按 ensure 响应的 role 分支:
   ├─ role=owner / write:
   │   ├─ git push origin main（remote URL = <tenant_gitea_base_url>/<kb_repo_path>）
   │   ├─ 使用调用者 fine-grained PAT（scope 含 costrict-kb repo write）
   │   └─ 推送成功 → done
   └─ role=none:
       ├─ 打印: "KB repo owned by <owner.display_name>(@<owner.username>)"
       ├─ 打印: hint
       ├─ 打印: "完成授权后重新执行: csc kb push"
       └─ exit code ≠ 0（便于脚本化判定）
```

#### 16.4.2 协作者授权流程（owner 视角）

owner 收到索权请求后，两种等价路径：

| 路径 | 操作 |
|---|---|
| Gitea Web UI | 进入 `costrict-kb/<kb-repo>` → Settings → Collaborators → Add（write 权限） |
| `csc kb authorize <username>` | csc 内部先 ensure（校验 role=owner）→ 调 `PUT /repos/.../collaborators/<username>` 加 write 权限 |

协作者重新 `csc kb push` → ensure 命中 role=write → push 成功。

#### 16.4.3 协作者撤销 / 转让

| 操作 | 命令 / 路径 | 调用权限 |
|---|---|---|
| 撤销协作者 | `csc kb revoke <username>` → `DELETE /repos/.../collaborators/<username>` | owner |
| 转让 ownership | `csc kb transfer-owner <username>` → `POST /repos/.../transfer`（owner 改为新用户，repo 留在 `costrict-kb/` org） | owner |

> 转让后原 owner 自动降级为 collaborator（write 权限），不丢失访问能力。

#### 16.4.4 KB 内容更新流程

由独立的 KB 生成服务负责（不在本规范范围）。该服务持独立 PAT，对 KB repo 有 write 权限（通过 owner 显式授权或 admin PAT 直配），按 KB 内容生成 pipeline 周期 push 更新。
...
- 所有命令**统一先调 `POST /api/kb/ensure`** 拿到 `kb_repo_path` + `role`
- `role=none` 时除 `csc kb list` 外全部前置失败并打印 owner
...
| 命令 | 行为 |
|---|---|
| `csc kb push` | ensure → role≠none → `git push origin main` |
| `csc kb pull` | ensure → role≠none → `git fetch && git pull origin main` |
| `csc kb status` | ensure → role≠none → 对比本地工作区与远端 main 的 commit diff |
| `csc kb list` | 调 `GET /repos/costrict-kb?limit=...`（Gitea API，按调用者可见范围过滤）→ 列出调用者参与的所有 KB repo |
| `csc kb authorize <username>` | ensure（必须 role=owner）→ `PUT /repos/.../collaborators/<username>` body `{permission:"write"}` |
| `csc kb revoke <username>` | ensure（必须 role=owner）→ `DELETE /repos/.../collaborators/<username>` |
| `csc kb transfer-owner <username>` | ensure（必须 role=owner）→ `POST /repos/.../transfer` body `{new_owner:"<username>"}` |
| `csc kb pr open/list/merge/close` | Gitea PR API 透传（不强制经过 ensure，但建议先 ensure 拿 path） |
...
| Force push | **禁止**（与 §7.3 main 分支保护对齐） |
| 删除 main | **禁止** |
...
| JWT tenant_id 校验 | ensure 接口前置校验（§1.5.2），不匹配 → 403 |
...
| ensure 调用本身 | costrict-web server access log（与所有 API 一致） |

> server 不维护独立 KB 审计表——所有审计能力由 Gitea 原生 audit log 提供。
...
| PAT 类型 | 对 `costrict-kb/` 的权限 |
|---|---|
| 用户 fine-grained PAT | 仅可对**已显式授权的 KB repo**做 push / pull / clone；**不允许 `costrict-kb` org 层的 repo:create scope** |
| admin PAT（`costrict-system`） | 全权：repo 创建 / collaborator 管理 / branch protection 配置 |
| KB 生成服务 PAT | 由 admin 单独签发，scope 限定为 `costrict-kb/*` write（owner 显式授权或 admin 直配，不在本规范范围） |

---

## 17. Workflow 仓库管理与协作（v2.16）

> **定位**：Workflow 是**业务数据**，不是 capability item type，不进 `capability_items` 表、不进 sync worker、不进 SecurityScan、不进 portal 列表。本规范仅约束 workflow 实例 repo 的**命名空间、创建、归属、协作**。Workflow 定义本身（节点编排 / DAG）由独立的平台 workflow 编排器负责，不在本规范范围。

### 17.1 设计原则

1. **实例 → repo 一一对应**：每个 workflow 实例独立一个 repo，**不允许跨实例共享**（C 方案"类型 repo + 实例 branch"已被否决：权限粒度只能到类型级、branch 不能删、PR 流程嵌套、路径反推破坏纯函数；详细对比见 v2.16 设计讨论纪要）
2. **路径推导纯函数化**：wf repo path 由 server 端纯函数从 `(workflow_def_slug, instance_id)` 推导（详见 [`WORKFLOW_REPO_PATH_ALGORITHM.md`](./WORKFLOW_REPO_PATH_ALGORITHM.md)），**不维护绑定关系表**
3. **server 仅 init**：`POST /api/workflow/init` 接口承担"判存在 + 首次创建 + 写入实例元数据 + 写入定义快照 + owner 授权 + 配置 branch protection"六件事，**其他全部回归 git 原生协作**（节点 push / PR / merge / collaborator / branch 均走 Gitea 原生 API）
4. **无 server 状态**：server 不持久化 workflow 实例元数据（instance.json 落 git），不维护 `wf_instances` 表，不算算法缓存——每次 init 实时计算 + 实时查 Gitea
5. **无权场景透明化**：调用者对已有 wf repo 无 collaborator 关系时，server 在 init 响应中返回 owner 信息（用户名 + 显示名），由调用者线下联系 owner 索权
6. **平台 namespace + owner 字段**：repo 落 `costrict-workflow/`（与 KB 一致的业务数据 namespace），归属语义通过 `.workflow/instance.json.owner` 字段记录触发者；用户 namespace `u-<username>/` 不混杂 workflow repo（D 方案"实例归属用户 namespace"已被否决：路径反推破坏纯函数、平台级 workflow 无自然 owner、跨用户列表扫描复杂；详细对比见 v2.16 设计讨论纪要）

### 17.2 org 配置：`costrict-workflow/`

| 字段 | 值 | 说明 |
|---|---|---|
| org name | `costrict-workflow` | 全 tenant 共享固定名（与 `costrict-official` / `costrict-template` / `costrict-config` / `costrict-mirror` / `costrict-kb` 同位） |
| visibility | **private**（强制） | workflow 实例内容仅协作团队可见，不暴露给 tenant 内其他用户 |
| `members_can_create_repos` | **false** | 用户不能在 UI 直接建 repo；所有 repo 创建走 server admin PAT |
| team 结构 | **无 team** | 不维护 `wf-creators` 等团队；权限模型直接落到 collaborator |
| 成员列表 | **空**（org 层面无成员） | 所有权限通过 repo 级 collaborator 表达 |
| SSO 自动加入 | **否** | 用户首次 init 时由 server admin PAT 临时加为 collaborator（仅对本次创建的 repo 生效），不影响其他 repo |
| webhook | 复用 tenant `costrict-config/platform-config` 中 `webhook_secret_ref` | workflow repo push / PR webhook 投递到独立的 workflow 编排器（不在本规范范围） |

### 17.3 server API：`POST /api/workflow/init`

唯一接口。**非幂等**（语义为"实例初始化"，重复 init 同 instance_id 但 definition 不同时拒绝）。

#### 17.3.1 请求

```http
POST /api/workflow/init HTTP/1.1
Authorization: Bearer <调用者 JWT>
Content-Type: application/json

{
  ...
  "audit_config": {                           // 可选，覆盖默认节点级策略
    "default_audit_level": "auto",            // strict | auto | experimental
    "node_overrides": {
      "003-deliver": "strict"                 // 关键节点强制 PR + 人工 approve
    }
  }
}
```
...
| 场景 | server 行为 | role | created | owner | hint |
|---|---|---|---|---|---|
| **不存在**（首次 init） | ① 用 `costrict-system` admin PAT 调 `POST /admin/users/costrict-workflow/repos` 建 repo（private） ② 写入 `.workflow/instance.json`（含 owner / 起止时间 / 状态=running） ③ 写入 `.workflow/definition.snapshot.yaml`（请求体中的 definition_snapshot） ④ 写入 `.workflow/node-prs.json`（空索引 `{}`） ⑤ 写入 `.workflow/audit-config.json`（audit_config 或默认值） ⑥ 调 `POST /repos/.../collaborators/<caller>` 把调用者加为 owner permission ⑦ 调 `POST /repos/.../branch_protections` 配置 main 分支保护（禁 force push / delete；允许 merge commit） | `owner` | true | 调用者自身 | 空 |
| **存在 + 调用者可访问** | 调 `GET /repos/.../collaborators/<caller>/permission` 反查权限 | `owner` / `write` | false | 实际 owner | 空 |
| **存在 + 调用者无权** | **不创建、不改权限**；调 `GET /repos/...` 取 owner username → 调 `GET /users/<owner>` 取 display_name | `none` | false | 实际 owner | `"Workflow repo 已由 <owner.display_name>(@<owner.username>) 创建，请通过站内消息 / IM / email 联系其授予 collaborator write 权限后重试"` |

#### 17.3.4 错误码

| HTTP | 触发条件 | 响应体 |
|---|---|---|
| 400 | `workflow_def_slug` 为空 / `instance_id` 非 UUID / `definition_snapshot` 缺失（详见算法 spec §4.4） | `{ "error": "invalid_request", "detail": "..." }` |
| 401 | 调用者未认证（JWT 缺失 / 过期） | `{ "error": "unauthorized" }` |
| 403 | JWT tenant_id 与本实例归属 tenant 不一致（§1.5.2） | `{ "error": "tenant_mismatch" }` |
| 409 | repo 已存在 + 调用者是 owner + `definition_snapshot` 与既有 `.workflow/definition.snapshot.yaml` hash 不一致（防定义漂移覆盖） | `{ "error": "definition_drift", "detail": "existing=<hash_existing> incoming=<hash_incoming>" }` |
| 500 | admin PAT 调 Gitea 失败（建 repo / 写文件 / 加 collaborator / 配保护） | `{ "error": "gitea_api_failed", "detail": "..." }` |

#### 17.3.5 重复 init 处理

- 同一调用者重复 init 同一 `(workflow_def_slug, instance_id)` + 相同 definition_snapshot：进入"存在 + 可访问"分支，返回 `role=owner / created=false`（允许平台编排器重试）
- 同一调用者重复 init 同一 `(workflow_def_slug, instance_id)` + 不同 definition_snapshot：返回 409 `definition_drift`（防止实例运行中定义被覆盖）
- 不同调用者 init 同一 instance_id：进入"存在 + 无权"分支，返回 owner 信息

### 17.4 实例 repo 结构

```
costrict-workflow/<def_slug>__<inst_short_id>/
├── .workflow/
│   ├── instance.json              # 实例元数据（owner / 起止时间 / 状态）
│   ├── definition.snapshot.yaml   # workflow 定义快照（init 时锁定，不可改）
│   ├── audit-config.json          # 节点级 PR 策略（strict / auto / experimental）
│   └── node-prs.json              # 节点 PR 索引：{ "001-collect": 12, "002-process": 13 }
├── nodes/
│   ├── 001-<node-slug>/
...
**关键约束**：
- `nodes/` 下目录以 `<3位序号>-<slug>` 命名，按时间线排序
- `definition.snapshot.yaml` **后续不可改**（防定义漂移导致审计失真）
- `node-prs.json` 由 server 在节点 PR 创建后自动追加，作为节点 → PR 反查索引
- `instance.json` 中 `status` 字段：`running` → `completed` / `failed` / `archived`（实例结束时由 server 写入）
...
```
节点执行器（agent / 用户）
  ├─ git checkout -b node/<seq>-<slug>              # base = main
  ...
  │    body:   节点 input/output 摘要 + 变更说明 + 关联上游节点 commit SHA
  ├─ reviewer 审计 → approve / comment / request_changes
  ├─ merge → main commit history 保留节点时间线
  └─ server 同步 .workflow/node-prs.json（追加 seq → PR number）
```

**Merge 策略**：强制 **merge commit**（不用 squash / rebase），保证节点 commit SHA 在主历史可追溯。

**节点级 PR 策略分级**（audit_config，详见 §17.3.1）：
...
| `experimental` | 不开 PR，留 branch 不并 main（实验性 / 探索性节点） | 无 |

策略来源：workflow 定义中每个 node 显式声明 `audit_level`，server 在 init 时读入并写入 `.workflow/audit-config.json`；默认值 `auto`。
...
```
1. 平台编排器启动实例时调 POST /api/workflow/init → 拿到 wf_repo_path + role=owner
2. 节点执行器（agent / 用户）：
   ├─ csc wf node push <seq>
   │   ├─ 内部先 init（幂等校验 role）
   │   ├─ git checkout -b node/<seq>-<slug>
   │   ├─ git push origin node/<seq>-<seq>
   │   └─ POST /repos/.../pulls（按 audit_level 决定是否自动 merge）
   └─ reviewer（audit_level=strict 时）：
       └─ csc wf node approve <pr-number>
```

#### 17.6.2 协作者授权流程（owner 视角）

owner 通过 Gitea Web UI 或 csc 命令加节点执行器 / reviewer：

| 路径 | 操作 |
|---|---|
| Gitea Web UI | 进入 `costrict-workflow/<wf-repo>` → Settings → Collaborators → Add（write 权限） |
| `csc wf authorize <username>` | csc 内部先 init（校验 role=owner）→ 调 `PUT /repos/.../collaborators/<username>` 加 write 权限 |

#### 17.6.3 协作者撤销 / 转让

| 操作 | 命令 / 路径 | 调用权限 |
|---|---|---|
| 撤销协作者 | `csc wf revoke <username>` → `DELETE /repos/.../collaborators/<username>` | owner |
| 转让 ownership | `csc wf transfer-owner <username>` → `POST /repos/.../transfer`（owner 改为新用户，repo 留在 `costrict-workflow/` org；同时更新 `.workflow/instance.json.owner` 字段） | owner |

### 17.7 实例生命周期

| 阶段 | repo 状态 | instance.json.status |
|---|---|---|
| 实例启动 | init 创建 repo，写入 instance.json + definition.snapshot.yaml + audit-config.json + 空 node-prs.json | `running` |
| 节点执行中 | 持续 PR + merge，main 实时累积 | `running` |
| 实例完成 | server 在 instance.json 标 `status=completed`，写入 `completed_at` | `completed` |
| 长期归档 | server 触发 Gitea repo archived（read-only）；branch protection 强化（禁止任何新 commit） | `archived` |
| 物理删除 | admin 显式操作（极端场景） | （删除） |

**保留期**：默认永久保留（审计意义大于存储成本），仅 admin 可显式删除。归档操作可由平台编排器在实例完成 N 天后自动触发（N 可配置，默认 30 天）。
...
| 层级 | 载体 | 用途 |
|---|---|---|
| **commit 历史** | `git log main` | 节点时间线（merge commit 含节点元信息） |
| **PR 链接** | `.workflow/node-prs.json` + Gitea PR 页面 | 节点 input/output 摘要 + reviewer + 评审讨论 |
| **跨节点数据流** | 每个节点的 `input.json` 记录上游节点 commit SHA | 上溯任何一个交付物的来源链 |

**典型溯源查询**："最终报告里这段结论从哪来？"

```
1. git log nodes/003-deliver/final-report.md → 找到 merge commit
2. node-prs.json["003-deliver"] → PR #15
3. PR body 中 input.upstream = "002-process@abc123"
4. git show abc123:nodes/002-process/output.md → 上游节点内容
5. 递归直到根节点
```
...
- 所有命令**统一先调 `POST /api/workflow/init`** 拿到 `wf_repo_path` + `role`（init 内部幂等：相同输入 + 相同 definition → 返回 created=false；不同 definition → 透传 409 错误）
- `role=none` 时除 `csc wf list` 外全部前置失败并打印 owner
- 不内置路径算法（path 来自 init 响应）
- 不维护本地缓存（每次命令重新 init，幂等成本低）

命令清单：

| 命令 | 行为 |
|---|---|
| `csc wf init <def_slug> <instance_id>` | 调 `POST /api/workflow/init`（首次必须含 `--definition-snapshot=<path>` / `--audit-config=<json>`） |
| `csc wf node push <seq>` | init → 切分支 node/`<seq>`-`<slug>` → push → 开 PR（按 audit_level 自动 merge） |
| `csc wf node list` | init → 列实例所有节点 + PR 状态（读 `.workflow/node-prs.json`） |
| `csc wf node approve <pr-number>` | init（必须 role=owner 或 reviewer）→ 调 `POST /repos/.../pulls/<n>/reviews` approve |
| `csc wf node merge <pr-number>` | init（必须 role=owner 或 reviewer）→ 调 `POST /repos/.../pulls/<n>/merge`（merge commit） |
| `csc wf list [--mine]` | 调 `GET /repos/costrict-workflow?limit=...`（Gitea API，按调用者可见范围过滤）；`--mine` 进一步按 `instance.json.owner` 过滤当前用户 |
| `csc wf authorize <username>` | init（必须 role=owner）→ `PUT /repos/.../collaborators/<username>` body `{permission:"write"}` |
| `csc wf revoke <username>` | init（必须 role=owner）→ `DELETE /repos/.../collaborators/<username>` |
| `csc wf transfer-owner <username>` | init（必须 role=owner）→ `POST /repos/.../transfer` body `{new_owner:"<username>"}` + 更新 `.workflow/instance.json.owner` |
| `csc wf archive` | init（必须 role=owner 或 admin）→ Gitea archived flag + instance.json.status=archived |
| `csc wf pr open/list/merge/close` | Gitea PR API 透传（不强制经过 init，但建议先 init 拿 path） |

### 17.10 分支与可见性策略

| 维度 | 规则 |
|---|---|
| 分支 | **主跟踪 main**；节点级 feat 分支 `node/<seq>-<slug>`（merge 后保留作历史） |
| Force push | **禁止**（与 §7.3 main 分支保护对齐） |
| 删除 main | **禁止** |
| 删除 node 分支 | merge 后默认保留（溯源需要）；admin 可显式清理（仅在归档后） |
| Merge 策略 | **强制 merge commit**（禁 squash / rebase，保留节点 commit SHA） |
| visibility | **恒定 private**（不允许 owner 改 public；防 workflow 业务数据外泄） |
| fork | 不允许 fork 到 `u-<username>/`（workflow 是业务产物，非 capability） |

### 17.11 多租户约束

| 场景 | 处理 |
|---|---|
| 同一 `(workflow_def_slug, instance_id)` 在 tenant A 与 tenant B 各自 init | 两 tenant 各自创建独立 wf repo（数据隔离；与 §1.5.5 一致） |
| 跨 tenant 协作 | **不可达**——不同 Gitea 实例之间无原生关系；如需跨 tenant 共享，走"显式打包迁移" |
...
| PAT 类型 | 对 `costrict-workflow/` 的权限 |
|---|---|
| 用户 fine-grained PAT | 仅可对**已显式授权的 wf repo**做 push / pull / clone / 开 PR / merge；**不允许 `costrict-workflow` org 层的 repo:create scope** |
| admin PAT（`costrict-system`） | 全权：repo 创建 / 文件写入 / collaborator 管理 / branch protection 配置 / archive |
| 平台 workflow 编排器 PAT | 由 admin 单独签发，scope 限定为 `costrict-workflow/*` write（用于节点 PR 自动 merge + 实例状态更新；不在本规范范围） |

### 17.15 与 KB（§16）的设计一致性

| 维度 | KB（§16） | Workflow（§17） |
|---|---|---|
| namespace | `costrict-kb/` | `costrict-workflow/` |
| 路径推断输入 | `code_repo_url` | `(workflow_def_slug, instance_id)` |
| server API | `POST /api/kb/ensure`（幂等） | `POST /api/workflow/init`（首次 init + 重试幂等 + 定义漂移 409） |
| repo 结构 | KB 生成服务自定义 | 强制 `.workflow/` 元目录 + `nodes/<seq>-<slug>/` 节点目录 |
| 默认协作模式 | 直推 main | 节点级 PR（按 audit_level 分级） |
| 生命周期 | 跟随代码 repo | 实例完成 → archived |
| 数据流 | KB 生成服务 push | 节点执行器 push + reviewer 审计 |
| 归属语义 | 共享（同代码 repo 用户） | owner 字段记录触发者（init 调用者） |

两者共用 v2.15 / v2.16 设计哲学：**server 仅预创建 + 纯函数路径推断 + 回归 git 原生协作 + 零 server 状态**。
