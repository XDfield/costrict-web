...
func buildNormalizeSystemPrompt(recent []ChatMessage) string {
	...
	b.WriteString("当 is_injection=false 时：把用户输入重写为清晰、规范的表达。\n")
	b.WriteString("- **只做语言层面的清理**：修正口语化、错别字、不必要缩写、语病，让指令更清楚\n")
	b.WriteString("- **绝对不要**在 rewritten 里编造用户没说过的具体内容，包括但不限于：\n")
	b.WriteString("  - 权限 ID / 问题 ID（如 perm-1、question-2 等）—— 用户不会写这些，你也不要替 ta 编\n")
	b.WriteString("  - 设备名 / 会话名 / 工作区名（如 DESKTOP-XXX、dev-laptop 等）\n")
	b.WriteString("  - 命令、文件路径、URL 等技术细节\n")
	b.WriteString("  - 具体动作描述（如「执行 rm 命令」「跑 ls」）\n")
	b.WriteString("- 上下文只能用来理解用户**指代什么**（「确认」→「确认」即可，不需要补「确认上一条权限申请 perm-1」），不能用来**编造**用户没说过的细节\n")
	b.WriteString("- 保持原意，不增删信息，不替用户做决定，不改语气\n")
	b.WriteString("- 已经清晰规范的输入原样返回\n")
	b.WriteString("反例（错误示范，**禁止**这样写）：\n")
	b.WriteString("  - 用户输入「自动允许啊」 → 错误：rewritten=\"自动批准来自 DESKTOP-XXX 的权限申请 perm-1，执行 cd /xxx\"\n")
	b.WriteString("  - 用户输入「批准」 → 错误：rewritten=\"批准 dev-laptop 上的 rm 操作\"\n")
	b.WriteString("正确示范：\n")
	b.WriteString("  - 用户输入「自动允许啊」 → rewritten=\"自动允许\" 或原样 \"自动允许啊\"\n")
	b.WriteString("  - 用户输入「确认」 → rewritten=\"确认\"\n")
	b.WriteString("  - 用户输入「批准一下那个」 → rewritten=\"批准那个\"\n")
	b.WriteString("  - 用户输入「不」 → rewritten=\"不\"\n")
