...
func NewRouter(cfg *config.Config, deps Deps) *gin.Engine {
	...
	// Public API endpoints — authenticated but not internal-token gated.
	// IdP sources API (E2) — managed by platform admins via UI.
	public := r.Group("/api")
	registerIdPRoutes(public, deps)
...
// registerIdPRoutes wires IdP sources CRUD endpoints (E2).
// When deps.IdPSources is nil, returns 503 so the swagger spec stays consistent.
func registerIdPRoutes(rg *gin.RouterGroup, deps Deps) {
	var svc handlers.IdPService
	if deps.IdPSources != nil {
		svc = deps.IdPSources
	} else {
		svc = unavailableIdPService{}
	}

	api := handlers.IdPSourcesAPI{Svc: svc}

	idpGroup := rg.Group("/idp-sources")
	{
		idpGroup.POST("", api.CreateIdPSource)
		idpGroup.GET("/:tenant_id/:provider", api.GetIdPSource)
		idpGroup.GET("/:tenant_id", api.ListIdPSources)
		idpGroup.PUT("/:tenant_id/:provider", api.UpdateIdPSource)
		idpGroup.DELETE("/:tenant_id/:provider", api.DeleteIdPSource)
		idpGroup.GET("/:tenant_id/enabled", api.GetTenantIdPs)
	}
}

// unavailableIdPService is the fallback when Deps.IdPSources is nil —
// keeps /api/idp-sources/* resolvable for swagger while refusing traffic with 503.
type unavailableIdPService struct{}

func (unavailableIdPService) Create(_ context.Context, _ idp.CreateParams) (*idp.IdPSourceView, error) {
	return nil, fmt.Errorf("IdP service not available")
}
func (unavailableIdPService) Get(_ context.Context, _, _ string) (*idp.IdPSourceView, error) {
	return nil, fmt.Errorf("IdP service not available")
}
func (unavailableIdPService) List(_ context.Context, _ string) ([]idp.IdPSourceView, error) {
	return nil, fmt.Errorf("IdP service not available")
}
func (unavailableIdPService) Update(_ context.Context, _ idp.UpdateParams) (*idp.IdPSourceView, error) {
	return nil, fmt.Errorf("IdP service not available")
}
func (unavailableIdPService) Delete(_ context.Context, _, _ string) error {
	return fmt.Errorf("IdP service not available")
}
func (unavailableIdPService) GetTenantIdPs(_ context.Context, _ string, _ idp.TenantConfigProvider) ([]idp.IdPSourceView, error) {
	return nil, fmt.Errorf("IdP service not available")
}

// registerInternalIdPRoutes wires server-to-server IdP endpoints that return
// raw configs (including secrets). These MUST stay behind RequireInternalToken.
func registerInternalIdPRoutes(rg *gin.RouterGroup, deps Deps) {
	var svc handlers.IdPService
	if deps.IdPSources != nil {
		svc = deps.IdPSources
	} else {
	...
	api := handlers.IdPSourcesAPI{Svc: svc}
	internal := rg.Group("/idp-sources")
	{
		internal.GET("/raw/:tenant_id", api.GetTenantIdPsInternal)
		internal.GET("/raw/:tenant_id/:provider", api.GetIdPSourceInternal)
	}
}
