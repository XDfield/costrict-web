...
> **算法 output vs HTTP 响应**：本算法只产出 `wf_repo_path`（路径段，与 Gitea 实例无关）。HTTP 端点层 `POST /api/internal/workflow/init` 在算法外多拼两个字段：
> - `wf_clone_url` = `<tenant_gitea_base_url>/<wf_repo_path>.git` — 编排器 `git clone` / `push` / `pull` 直接用，**禁止**客户端自行拼 base_url（详见 [`CSC_WF_SUBCOMMAND_CONTRACT.md`](./CSC_WF_SUBCOMMAND_CONTRACT.md) §1.5 与 [`REPOSITORY_MANAGEMENT_SPEC.md`](./REPOSITORY_MANAGEMENT_SPEC.md) §10.3.1 的 SoT 约定）
> - `wf_web_url` = `<tenant_gitea_base_url>/<wf_repo_path>` — 浏览器跳转用
>
...
3. server 用 admin PAT 调 Gitea:
   ...
   - 404 → 类型 repo 不存在分支:
     ...
     返回 {
       ...
       instance_branch: "inst-f3a8b2c1",
       created: {type_repo: true, instance_branch: true},
   ...
   - 200 → 类型 repo 存在:
     ...
     - 一致 →
       ...
       返回 { wf_repo_path, wf_clone_url, wf_web_url, instance_branch, created: {type_repo: false, instance_branch: <bool>}, algorithm_version: "v2" }
   - team ns 不存在（org 不存在）→ 412 TEAM_NS_NOT_INITIALIZED

4. csc 按响应行为:
   - team_ns_exists=true → 节点执行器后续切分支 node/<seq>-<slug>, base=inst-f3a8b2c1（remote = response.wf_clone_url）
