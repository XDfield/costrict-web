...
func NewRouter(cfg *config.Config, deps Deps) *gin.Engine {
	...
	// Swagger UI is gated by CS_USER_SWAGGER_ENABLED (default false). The
	// spec is generated by `make swagger` (swag init) and registered
	// globally via the blank import of cs-user/docs in main.go. Production
	// deployments leave this off — cs-user is cluster-internal and the
	// spec enumerates 33 internal endpoints plus the X-Internal-Token
	// header name. Dev / CI set CS_USER_SWAGGER_ENABLED=true to mount the
	// UI for local iteration. The generated spec is unaffected.
