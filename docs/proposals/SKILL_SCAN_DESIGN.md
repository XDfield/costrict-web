> **实现状态：后端已完成**
>
> - 状态：✅ 后端已完成；前端展示与分类字典消费仍可继续收敛
> - 实现位置：`server/internal/services/`（`scan_service.go`, `scan_job_service.go`, `sync_service.go`）；`server/internal/worker/scan_worker.go`；`server/internal/handlers/scan.go`；`server/internal/models/models.go`（`SecurityScan` / `ScanJob` / `CapabilityItem`）
> - 说明：能力项安全扫描后端已实现，包括扫描任务调度、异步 Worker 执行、HTTP 接口、`sync` 后自动触发扫描，以及扫描时自动分类并回写 `capability_items.category`。
...
能力项创建 / 同步写入
  ...
  └─ 输出结构化扫描报告 + category slug
        ↓
  写入 security_scans 表
  更新 capability_items.security_status / last_scan_id / category
        ↓
  前端展示扫描徽章 / 详情
```

复用 `SKILL_SYNC_DESIGN.md` 中已有的 **PostgreSQL 队列 + Worker Pool** 模式（`SELECT FOR UPDATE SKIP LOCKED`），扫描任务与同步任务共享同一套基础设施，但队列表独立。当前实现使用的实际表名为 `scan_jobs` 与 `security_scans`。
...
当前实现复用了 `capability_items` 上已有扫描字段，实际使用的是以下字段：

```sql
ALTER TABLE capability_items ADD COLUMN security_status VARCHAR(32) DEFAULT 'unscanned';
-- unscanned : 未扫描
-- scanning  : 扫描中
-- clean     : 无风险
-- low       : 低风险
...
-- extreme   : 极高风险
-- error     : 扫描失败
-- skipped   : 跳过（预留状态，当前实现未主动写入）

ALTER TABLE capability_items ADD COLUMN last_scan_id UUID;
ALTER TABLE capability_items ADD COLUMN category VARCHAR(128);
```

说明：

- `security_status` 由 `ScanService.ScanItem()` 在扫描开始、成功、失败时更新
- `last_scan_id` 指向最近一次 `security_scans.id`
- `category` 在扫描成功后会被自动回写为 LLM 选出的分类 slug；仍支持通过现有 Item 更新接口手动修改

### 3.2 `security_scans` 表（扫描结果）

当前实现落库表名为 `security_scans`，结构化字段被拆成独立列，而不是统一的 `report JSONB`：

```sql
CREATE TABLE security_scans (
  id              UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  item_id         UUID NOT NULL REFERENCES capability_items(id) ON DELETE CASCADE,
  item_revision   INTEGER NOT NULL,           -- 对应 capability_versions.revision

  -- 扫描元信息
  trigger_type    VARCHAR(32) NOT NULL,       -- create | update | sync | manual
  scan_model      VARCHAR(128) NOT NULL,
  duration_ms     INTEGER,
  category        VARCHAR(128) NOT NULL DEFAULT '', -- 分类 slug，如 frontend-development
...
CREATE INDEX idx_security_scans_item_id ON security_scans(item_id);
CREATE INDEX idx_security_scans_risk_level ON security_scans(risk_level);
```

补充说明：

- `category` 为本次扫描得出的分类结果，来源于 LLM 输出
- 自动分类使用固定 taxonomy，由后端校验合法性
- taxonomy 当前通过迁移预置到 `item_categories`，见 `20260407110000_add_scan_category_and_seed_taxonomy.sql`

### 3.3 `scan_jobs` 表（扫描任务队列）

独立于同步任务队列（`sync_jobs`），避免相互干扰。当前实现表名为 `scan_jobs`：
...
CREATE TABLE scan_jobs (
  ...
  finished_at    TIMESTAMP,
  scan_result_id UUID,                   -- 完成后关联 security_scans.id

  created_at     TIMESTAMP DEFAULT NOW()
);

CREATE INDEX idx_scan_jobs_status ON scan_jobs(status, scheduled_at);
CREATE INDEX idx_scan_jobs_item_id ON scan_jobs(item_id);
```
...
在 `SyncService.SyncRegistry()` 的增量对比循环中，每次新增或更新一条 `CapabilityItem` 后，调用 `enqueueScan()` 追加入队。当前 `cmd/worker` 已显式注入 `ScanJobService`，因此 `sync` 后自动扫描链路已打通：

```
SyncService 写入 CapabilityItem（新增或更新）
        ↓
...
2. 构造扫描输入（元信息 + 完整内容）
3. 更新 capability_items.security_status = "scanning"
4. 调用 LLMClient.Complete(systemPrompt, userInput)
5. 解析 JSON 输出（失败则重试一次）
6. 校验 category 是否属于允许的 taxonomy
7. 写入 security_scans
8. 更新 capability_items.security_status = risk_level
9. 更新 capability_items.last_scan_id
10. 若 category 非空，回写 capability_items.category，并通过 CategoryService.EnsureCategory() 保证 item_categories 中存在该分类
```
...
| `GET` | `/api/items/:id/scan-status` | 返回 `security_status` 的当前值及最新结果摘要 |
| `GET` | `/api/items/:id/scan-results` | 历史扫描记录，含每次的 category / risk_level / verdict / trigger_type / created_at |
...
└──────────────────────────────────────────────────────────────┘
        ...
        ┌───────────────────────────────────────┐
        │  security_scans                       │
        │  capability_items.security_status 更新│
        └───────────────────────────────────────┘
                            ↓
...
| `SKILL_SYNC_DESIGN.md` SyncService | `SyncService.SyncRegistry()` 在写入 `CapabilityItem` 后调用 `enqueueScan()`，`cmd/worker` 中已注入 `ScanJobService`，`sync` 后自动扫描链路已落地 |
| `SKILL_DATA_DESIGN.md` capability_items | 实际复用 `security_status` / `last_scan_id` 字段，并在扫描成功后自动回写 `category` |
| `SKILL_DATA_DESIGN.md` item_categories | 通过 `20260407110000_add_scan_category_and_seed_taxonomy.sql` 预置分类 taxonomy，并在扫描回写分类时自动 `EnsureCategory()` |
...
| 阶段 | 内容 | 预估时间 |
|------|------|---------|
| **P0** | 后端：`scan_jobs` / `security_scans` / `ScanJobService` / `ScanWorkerPool` / 扫描 API | ✅ 已完成 |
| **P0** | 触发点接入：`CreateItem` / `UpdateItem` / `SyncService` 写入后入队 | ✅ 已完成 |
| **P0** | 扫描自动分类：固定 taxonomy、结果校验、回写 `capability_items.category` | ✅ 已完成 |
| **P1** | 前端：统一改为以后端 `item_categories` 为单一数据源 | 进行中 |
| **P1** | 前端：能力项列表扫描徽章 / 详情页安全扫描区块 | 部分已具备，仍可继续收敛 |
| **P1** | 前端：安装确认弹窗（caution / reject 场景） | 待完成 |
| **P2** | 管理员视图：全局扫描队列状态 / 高风险条目汇总 | 待完成 |
| **P2** | 自动归档策略：`extreme` 风险条目自动设为 `draft` 并通知管理员 | 待完成 |

当前可认为后端扫描能力已上线可用；前端展示与分类字典消费仍可继续按 P1 收口。
