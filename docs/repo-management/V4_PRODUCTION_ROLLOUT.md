...
> 验收方法见 `CAPABILITY_GIT_E2E_RUNBOOK.md`。本文只讲**怎么上线**。
...
| 3 | `GIT_CAPABILITY_WORKER_CONCURRENCY` | ❌ 缺失 | 默认并发过低。**本地实测**：周期巡检攒下 355 个 job 后，用户 push 触发的实时同步排到队尾要等一个多小时。建议 **8** |
...
⚠️ **worker 必须与 api 同批上线**。`content_md5` 的列宽 32→64 **没有 SQL migration**，
靠 worker 的 AutoMigrate（api 侧不跑）。worker 落后会导致 64 位 SHA-256 写入失败。
...
**全部是新增列/新建表，不删不改存量数据**，所以对存量行为零影响。
...
| 字段 | 值 | 谁在用 |
|---|---|---|
| `endpoint` | Gitea 的**集群内**地址 | api / worker 出站 |
| `config.web_url` | Gitea 的**浏览器可达**地址 | 用户点链接跳转 |

配成同一个值时：要么服务连不上，要么页面上全是打不开的链接。

推荐用 `GIT_SERVER_TEMPLATE_*` 环境变量让 api 启动时自动种子化，
而不是手工 INSERT——它是幂等的（已有 template 行时 no-op）。

### 6.2 实例级 system webhook

Gitea 侧需要一条指向 `<api>/api/internal/git-sync/<server_id>` 的 **push** webhook。
配了 `GIT_SYSTEM_WEBHOOK_BASE_URL` 后 worker 的 reconciler 会自动维护它。
...
否则 fork 探测不到会回落 DB。**大批量导入前必须先配 namespace 排除**，
