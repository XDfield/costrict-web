...
4. PostgreSQL 维护全部对象映射；每条非文本对象记录都同时保存
   `storage_backend + storage_key`，包括 asset、artifact 和 admin import upload job。
...
### 5.1 PostgreSQL 维护全部对象映射

非文本对象不能只保存 `storage_key`。所有持久化对象引用都必须同时记录：
